...
# Start training
for dataset in tqdm(
    config.datasets, total=len(config.datasets), ncols=70, leave=True, position=0
):
    ...
    if dataset == "SumMe":
        batch_size = (
            ...
            else int(config.SumMe_len * 0.8 * float(config.batch_size))
        )
    elif dataset == "TVSum":
        batch_size = (
            1
            if config.batch_size == "1"
            else int(config.TVSum_len * 0.8 * float(config.batch_size))
        )

    for split_id, (train_loader, test_loader) in tqdm(
        enumerate(create_dataloader(dataset)),
        total=5,
        ncols=70,
        leave=False,
        position=1,
        ...
        for epoch in tqdm(
            ...
            leave=False,
            position=2,
            desc=f"Split{split_id+1}",
        ):
            model.train()
            update_loss = 0.0
            batch = 0

            for feature, gtscore, dataset_name, video_num in tqdm(
                train_loader,
                ncols=70,
                leave=False,
                position=3,
                desc=f"Epoch{epoch+1}_TRAIN",
            ):
            ...
            with torch.no_grad():
                for feature, gtscore, dataset_name, video_num in tqdm(
                    ...
                    if dataset_name in ["SumMe", "TVSum"]:
                        with h5py.File(
                            f"drive/MyDrive/research/paper2/data/eccv16_dataset_{dataset_name.lower()}_google_pool5.h5",
                            "r",
                        ) as hdf:
                        ...
                        scores = (
                            output.squeeze().clone().detach().cpu().numpy().tolist()
                        )
                        summary = generate_summary(
                            [sb], [scores], [n_frames], [positions]
                        )[0]
                        if dataset_name == "SumMe":
                            spear, kendall = get_corr_coeff(
                                [summary], [video_num], dataset_name, user_summary
                            )
                        elif dataset_name == "TVSum":
                            spear, kendall = get_corr_coeff(
                                [scores], [video_num], dataset_name, user_scores
                            )
            ...
            if (
                np.mean(val_kendalls) > model_selection_kendall
                and np.mean(val_spears) > model_selection_spear
            ):
